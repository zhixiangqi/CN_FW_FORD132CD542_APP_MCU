...
void DisplayChipApp_FaultCheck(void)
{
  ...
  if(u8Status == ERROR_NONE){
    ...
    if(u8Status != ERROR_NONE){
        ...
        sprintf((char *)u8TxBuffer,"FAULT CHECK FLOW> LCD I2C ERROR=0x%02x [0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x]\r\n",u8Status,u8ASIL[0],u8ASIL[1],u8ASIL[2],u8ASIL[3],u8ASIL[4],u8ASIL[5]);
        ...
        sprintf((char *)u8TxBuffer,"FAULT CHECK FLOW> LCD [0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x]\r\n",u8ASIL[0],u8ASIL[1],u8ASIL[2],u8ASIL[3],u8ASIL[4],u8ASIL[5]);
        UartDriver_TxWriteString(u8TxBuffer);
    }
  }
  /*TSCERR*/
  ...
  u8Status = I2C4MDriver_Write(CHIP_ADDR,u8PageCmd,sizeof(u8PageCmd));
  if(u8Status == ERROR_NONE){
    u8Status = I2C4MDriver_WriteRead(CHIP_ADDR,u8ReadRegister0x1C,sizeof(u8ReadRegister0x1C),&u8ASIL[6],1U);
    if(u8Status != ERROR_NONE){
        DiagApp_I2CMasterFaultCheck(true,DIAG_I2CM_LCD_MASK);
